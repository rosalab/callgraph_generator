...
#define DISABLE_INDIRECTCALLS 0
...
void run(CustomCallGraph &CGPass){
	...
	errs()<< "Indirect-call Map Gen Started\n";
  CGPass.indirectCallMapGen();

	errs()<< "Indirect-call Map is populated\n";
	errs()<< "Printing Stats after ModulePass\n";
  #endif // DISABLE_INDIRECTCALLS
	// questions to answer
	...
	// Before passing helper functions lets resolve indirect calls
	// in the CallerCalleeMap
 #if 0

	errs()<< "Indirect-call Resolver Started\n";
  CGPass.indirectCallResolver();
  // resolves indirect calls from the CallerCalleeMap
	errs()<< "Indirect-call Resolver Completed\n";
  ...
  //std::string arr[] = {"bpf_cgrp_storage_get"};
